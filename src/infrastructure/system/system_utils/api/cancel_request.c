...
BVIEW_STATUS system_utils_json_cancel_request (void *cookie, char *jsonBuffer, int bufLength)
{
    ...
    REG_HB_JSON_CANCEL_REQUEST_t command;
    ...
    JSON_VALIDATE_JSON_POINTER(json_req_id, "request-id", BVIEW_STATUS_INVALID_JSON);
    JSON_VALIDATE_JSON_AS_NUMBER(json_req_id, "request-id");
    /* Copy the value */
    command.id = json_req_id->valueint;
    status = reg_hb_json_cancel_request_impl (cookie, asicId, id, &command);
