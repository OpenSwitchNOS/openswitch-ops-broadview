...
bst_system_bufmon_config_update (struct json *json_object)
{
  ...
  if (true == updated)
  {
    ...
    bst_prepare_configure_bst_feature_req(1, bufmon_config->bst_enable,
	bufmon_config->periodic_collection, 
	bufmon_config->collection_interval, 0, 0,
        bufmon_config->bstMaxTriggers,
        bufmon_config->sendSnapshotOnTrigger, 0, 0);
  }
  /* Release lock */
...
static 
BVIEW_STATUS bst_ovsdb_track_update_send(int trackingMask, int trackMode)
{

    int trackPeakStats = trackMode;
    int trackIngressPortPriorityGroup = false;
    int trackIngressPortServicePool = false;
    int trackIngressServicePool = false;
    int trackEgressPortServicePool = false;
    int trackEgressServicePool = false;
    int trackEgressUcQueue = false;
    int trackEgressUcQueueGroup = false;
    int trackEgressMcQueue = false;
    int trackEgressCpuQueue = false;
    int trackEgressRqeQueue = false;
    int trackDevice = false;
    int realmId;
    bool val;
  ...
  {
    val = false;
    if(trackingMask & (1 <<realmId))
    {
     val = true;
    switch (realmId)
    {
      case BVIEW_BST_DEVICE:
      trackDevice = val;
      break;

      case BVIEW_BST_EGRESS_PORT_SP:
      trackEgressPortServicePool = val;
      break;

      case BVIEW_BST_EGRESS_SP:
      trackEgressServicePool = val;
      break;

      case BVIEW_BST_EGRESS_UC_QUEUE:
      trackEgressUcQueue = val;
      break;

      case BVIEW_BST_EGRESS_UC_QUEUEGROUPS:
      trackEgressUcQueueGroup = val;
      break;

      case BVIEW_BST_EGRESS_MC_QUEUE:
      trackEgressMcQueue = val;
      break;

      case BVIEW_BST_EGRESS_CPU_QUEUE:
      trackEgressCpuQueue = val;
      break;

      case BVIEW_BST_EGRESS_RQE_QUEUE:
      trackEgressRqeQueue = val;
      break;

      case BVIEW_BST_INGRESS_PORT_PG:
      trackIngressPortPriorityGroup = val;
      break;

      case BVIEW_BST_INGRESS_PORT_SP:
      trackIngressPortServicePool = val;
      break;

      case BVIEW_BST_INGRESS_SP:
      trackIngressServicePool = val;
      break;
   ...
   bst_prepare_configure_tracking_req (1, trackPeakStats,
                               ...
                               bool initial)
{
  ...
  /* check if there is any diff in old and new track mask */
  {
          ...
          bst_ovsdb_track_update_send (trackMask, true);
  } 
